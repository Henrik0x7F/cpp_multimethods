...
/**
 * Wrapper of custom_multimethod with Predicate = TypeIdPredicate
 ...
 * If you also want to receive types which inherit from a specified type, use relaxed_multimethod
 * See custom_multimethod for documentation of parameters
 */
template<typename Handler, typename BaseType, typename... ConcreteTypes>
bool strict_multimethod(Handler handler, BaseType param)
{
    return Impl::double_dispatch<Handler, Impl::TypeIdPredicate, BaseType, ConcreteTypes...>(handler, param);
}

/**
 * Wrapper of custom_multimethod with Predicate = DynamicCastPredicate
 * Calls Handler::on_dispatch(Type) for every type whose runtime type is Type or a derivative of Type
 * See custom_multimethod for documentation of parameters
 */
template<typename Handler, typename BaseType, typename... ConcreteTypes>
bool relaxed_multimethod(Handler handler, BaseType param)
{
...
bool custom_multimethod(Handler handler, BaseType param, Predicate pred = Predicate())
{
    return Impl::double_dispatch<Handler, Predicate, BaseType, ConcreteTypes...>(handler, param);
}
